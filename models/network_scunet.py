...
class SCUNet_original(nn.Module):

    def __init__(self, in_nc=3, config=[2, 2, 2, 2, 2, 2, 2], dim=64, drop_path_rate=0.0, input_resolution=256):
        super(SCUNet_original, self).__init__()
        self.config = config
        self.dim = dim
        self.head_dim = 32
        self.window_size = 8
        ...
        self.m_down2 = [ConvTransBlock(dim, dim, self.head_dim, self.window_size, dpr[i + begin],
                                       'W' if not i % 2 else 'SW', input_resolution)
                        for i in range(config[1])] + \
                       [nn.Conv2d(2 * dim, 4 * dim, 3, 1, 1, bias=False)]

        begin += config[1]
        self.m_down3 = [ConvTransBlock(2 * dim, 2 * dim, self.head_dim, self.window_size, dpr[i + begin],
                                       'W' if not i % 2 else 'SW', input_resolution)
                        for i in range(config[2])] + \
                       [nn.Conv2d(4 * dim, 8 * dim, 3, 1, 1, bias=False)]
        ...
        begin += config[3]
        self.m_up3 = [nn.Conv2d(8 * dim, 4 * dim, 3, 1, 1, bias=False),] + \
        ...
        self.m_up2 = [nn.Conv2d(4 * dim, 2 * dim, 3, 1, 1, bias=False),] + \
        ...
        self.m_up1 = [nn.Conv2d(2 * dim, dim, 3, 1, 1, bias=False), ] + \
...
if __name__ == '__main__':
    ...
    net = SCUNet()
    for index,(name,paramerter) in enumerate(net.named_parameters()):
        print(index,name)
    # print(net)
